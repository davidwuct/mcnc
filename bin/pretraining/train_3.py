...
def train_sample_truncated_ng(ng_edges,
    ...
    for i in range(len(ng_edges)):
        ...
        for j in range(len(input_edges)):
            ...
            g = dgl.graph((new_edges[0].long(), new_edges[2].long()), num_nodes=n_nodes)
            ...
            for k in range(new_edges.shape[1]):
                nid2 = new_edges[2][k]
                new_edges[3][k] = 1.0 / g.in_degrees(nid2.long())
...
def train(rank, args):
    ...
    for i_epoch in range(args.n_epochs):
        ...
        for i_file, f in enumerate(train_files):
            ...
            logger.info('{} / {} files completed'.format(i_file + 1, len(train_files)))
