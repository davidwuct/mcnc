...
def worker(args, pidx, g_graph_counts, g_rel_counts, g_ntype_counts,
    ...
    chain_fpath = os.path.join(target_dir, '{}.json'.format(prefix)).replace('\\', '/')
    docid_fpath = os.path.join(target_dir, '{}.txt'.format(prefix)).replace('\\', '/')
    ...
    with open(src_fpath, 'r') as fr:
        for line in fr:
            ...
            if args.save_ng_pkl:
                ...
                dpath = os.path.join(target_dir, 'ng_pickles').replace('\\', '/')
                if not os.path.exists(dpath):
                    os.makedirs(dpath)
                fpath = os.path.join(dpath, '{}_{}.pkl'.format(
                    count_valid, doc['doc_id'])).replace('\\', '/')
    ...
    logger.info('Avg. # Nodes: {:.2f}'.format(sum(g_node_counts)/len(g_node_counts)))
...
def main(args):
    ...
    train_dir = os.path.join(config['nyt_dir'], 'train').replace('\\', '/')
    dev_dir = os.path.join(config['nyt_dir'], 'dev').replace('\\', '/')
    test_dir = os.path.join(config['nyt_dir'], 'test').replace('\\', '/')
    if args.target_split is None:
        train_fs = sorted([os.path.join(train_dir, fn).replace('\\', '/') for fn in os.listdir(train_dir)])[:1]
        dev_fs = sorted([os.path.join(dev_dir, fn).replace('\\', '/') for fn in os.listdir(dev_dir)])[:1]
        test_fs = sorted([os.path.join(test_dir, fn).replace('\\', '/') for fn in os.listdir(test_dir)])[:1]
        fs = train_fs + dev_fs + test_fs

    elif args.target_split == 'train':
        fs = sorted([os.path.join(train_dir, fn).replace('\\', '/') for fn in os.listdir(train_dir)])
    elif args.target_split == 'dev':
        fs = sorted([os.path.join(dev_dir, fn).replace('\\', '/') for fn in os.listdir(dev_dir)])
    elif args.target_split == 'test':
        fs = sorted([os.path.join(test_dir, fn).replace('\\', '/') for fn in os.listdir(test_dir)])
    else:
    ...
    for pidx, src_fpath in enumerate(fs):
        ...
        target_dir = os.path.join(args.output_dir, split).replace('\\', '/')
